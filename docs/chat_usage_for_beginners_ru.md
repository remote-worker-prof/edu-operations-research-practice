...
Важно: после hard cutover основной пользовательский интерфейс находится на `/app/`.
...
- нажать действие вроде "Показать результат" или "Решить";
- использовать slash-команды только при необходимости.
...
- В новом основном UI extension выбирается в guided panel нового React-чата.
- Выбранный extension привязывается к текущей сессии.
- Если сессия уже непустая, перед сменой extension надо сначала отправить `reset`.
...
Дополнение про extensions:

- по умолчанию стартует built-in extension `default_or`;
- в UI можно выбрать и другой extension, например `study_planner`;
- смена extension работает **на уровне сессии**, а не одного сообщения;
- если вы уже успели заполнить данные, сначала нужен `reset`, и только потом можно переключаться.
